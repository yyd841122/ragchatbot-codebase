...
重点检查 `### 计划修改文件` 章节，确保第一个文件是仓库中真实存在的 `README.md`。
...
在执行 `/zhipu-apply` 之前，请先确认 `### 计划修改文件` 的第一个文件是仓库中真实存在的 `README.md`。
...
### 2. Step 5 跳过非支持文件
- **常见原因**：第一个目标文件不是 `README.md`
- **解决方法**：在 Stage 1 计划中将 `README.md` 写在第一位
...
**Issue → 结构化计划 → 工作分支 → README.md 修改 → commit → Draft PR**
